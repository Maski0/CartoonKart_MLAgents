...
public class KartControllerPlayer : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        //Follow Collider
        transform.position = sphere.transform.position - new Vector3(0, -0.23f, 0);
        ...
        //Steer
        if (Input.GetAxis("Horizontal") != 0 && currentSpeed > 1f)
        {
        ...
        else if (Input.GetAxis("Horizontal") != 0 && currentSpeed < -1f)
        {
        ...
        //Drift
        if (Input.GetButtonDown("Jump") && !drifting && Input.GetAxis("Horizontal") != 0 && ismoving)
        {
            ...
            {
                ParticleSystem.MainModule pmain = p.main; //for refff https://answers.unity.com/questions/1331022/55-particle-startcolor-warning.html
    ...
    public void Boost()
    {
        ...
        foreach (ParticleSystem p in primaryParticles)
        {
            ParticleSystem.MainModule pmain = p.main; //https://answers.unity.com/questions/1331022/55-particle-startcolor-warning.html
            pmain.startColor = Color.clear;
            p.Stop();
        }
